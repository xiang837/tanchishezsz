...
QString MainWindow::getCurrentMapName() const {
    switch (mapid) {
    case 0: return "Borderless";
    case 1: return "Classic_Box";
    case 2: return "Trail_Station";
...
QString mapName = getCurrentMapName();
    int currentHigh = ScoreManager::getHighScore(mapName);
    bool isNewRecord = score > currentHigh;
    
    QString message = QString(sig==1 ? "You hit yourself!\n" : "You hit the wall!\n")
        .append(scorelabel.text())
        .append("\nMap High Score: ")
        .append(QString::number(currentHigh));
    
    if (isNewRecord) {
        ScoreManager::updateScore(mapName, score);
        message.append("\nNew Record!");
    }
